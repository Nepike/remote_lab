 ...
 *  \version 5.05
 *  \date 10.04.2014
 *  \date Last Change: 24.08.2017
 */

// Эти макросы определяют тип платформы
#define _PT_MPP_    // МПП
// #define _PT_MODEL_  // Лабораторный макет МПП
//#define _PT_LLW_      // Платформа с мотор-редукторами LLW (YARP2/tmu4)

/// Версия прошивки
#define IMVERSION 0x55
#define Title "MVCTL 5.05"
...
int CSPEED = 0;  /// Крейсерская скорость (PWM)
int RSPEED = 0;  /// Скорость разворота (PWM)

// Скорости движения и разворота
int mv_speed = 0;
int rot_speed = 0;
 ...
 *          (по паспорту - вдвое меньше, но т.к. мы регистрируем изменения сигнала, а не сам сигнал, то надо писать в 2 раза больше)
...
BYTE REG[NREG] = {
 ...
 IMVERSION,  /**<  1: REG_VERSION:    Версия */
        ...
        40,  /**< 11: REG_USS_DIST:   Дистанция срабатывания аналоговых (УЗД/Sharp) бамперов 150 */
         ...
         0,  /**< 12: REG_MINU:       minU (от 0 до 255) */
       250,  /**< 13: REG_MAXU:       maxU (от 0 до 255) */
         0,  /**< 14: REG_MINSPEED:   minSpeed - Скорость трогания */
       100,  /**< 15: REG_MAXSPEED:   maxSpeed - Определяется процедурой DefineMaxSpeed(pidlib::maxU) */
       200,  /**< 16: REG_PWMPEED:    WORK_pwmspeed - Скорость движения без ПИД-управления */
...
void Regs2CtlVars(void)
{
  ...
  CSPEED = (WORK_pwmspeed*2/3);                // Крейсерская скорость (maxSpeed*3/4)
...
void setup()
{
   ...
   */
  float Kp = 0.1, Ki = 0.35, Kd = 0; // 0.5 0.52 0.0.5

// pidlib::maxSpeed. Максимальная скорость движения. Величина определяется с помощью процедуры DefineMaxSpeed
#ifdef _PT_MPP_
  pidlib::maxSpeed = 27;
#endif
  ...
  mv_speed = pidlib::maxSpeed/2;
  rot_speed = mv_speed/4;

  robot.InitPID(Kp, Ki, Kd, -pidlib::maxSpeed*2, pidlib::maxSpeed*2, (float)pidlib::minU, (float)pidlib::maxU, pidlib::maxErr);

  // Геометрия и параметры энкодеров
  // Период прерываний таймера (в микросекундах)
  // Например, tns = 50'000 микросекунд (или 0.05 сек - или 20Hz)

#ifdef _PT_MPP_
  unsigned long tns = 400000;
#endif

#ifdef _PT_MODEL_
  unsigned long tns = 300000;
#endif

#ifdef _PT_LLW_
  unsigned long tns = 100000;
#endif
  ...
  // *** Ручной тест энкодеров
  //robot.TestEncoders();

  // *** Определение максимальной скорости при управлении U
  // Функция нужна для определения параметра pidlib::maxSpeed.
  // Для этого нужно вызывть ее с аргументом pidlib::maxU (DefineMaxSpeed(pidlib::maxU))
  // и далее посмотреть в терминале выдаваемые значения. Это и будет величина maxSpeed
  //DefineMaxSpeed(pidlib::maxU);


  // *** Простой тест ПИД-регуляторов
  //robot.TestPID(-rot_speed, rot_speed);

  // *** Комплекстный тест ПИД-регуляторов
  //TestComplexPID(mv_speed, rot_speed);
...
/*
// !!!!! REMOVE THIS AFTER DEBUGGING PLEASE !!!!!
  lcd.setCursor(0, 1);
  lcd.print(p_part);
  lcd.print(" ");
  lcd.print(i_part);
  lcd.print(" ");
  lcd.print(d_part);
// !!!!!!! END OF REMOVE THIS AFTER DEBUGGING PLEASE !!!!!
*/
  ...
  if(rcReadPackage(&pckg)==DATA_READY)
  {
    ...
    REG[REG_D1] = robot.Cnt2L(stdrobot::GlobalEncoderLeftCnt);
    REG[REG_D2] = robot.Cnt2L(stdrobot::GlobalEncoderRightCnt);
    switch(CMD)
    {
      case CMD_PING:
        break;
      ...
      case CMD_FWD2:
        if(obst) break;
        rcGet1B(&pckg, &val);
        robot.reStartMove(10*val, mv_speed, mv_speed);
        break;
      case CMD_BACK:
        goBack(CSPEED);
        break;
      ...
      case CMD_FAST_LEFT2:
        rcGet1B(&pckg, &val);
        angle = val;
        robot.reStartRotate(angle, rot_speed);
        break;
      case CMD_FAST_RIGHT:
        goFastRight(RSPEED);
        break;
      ...
      case CMD_SET_SPEED:
        rcGet2B(&pckg, &b1, &b2); 
        robot.GoalSpeedLeft =  ((int)pidlib::maxSpeed*(signed char)b1/100);
        robot.GoalSpeedRight = ((int)pidlib::maxSpeed*(signed char)b2/100);
        break;
      case CMD_BEEP:
        Beep(1);
        break;
      ...
      case CMD_SET_REG:
        ...
        if(n==REG_D1) stdrobot::GlobalEncoderLeftCnt = 0;
        if(n==REG_D2) stdrobot::GlobalEncoderRightCnt = 0;
        Regs2CtlVars();
        break;
      case CMD_GET_REG:
        rcGet1B(&pckg, &n);
