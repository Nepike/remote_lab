...
volatile int stdrobot::CTIME = 0; /**Счетчик времени до остановки робота после потери связи**/
...
void CtlSpeed(TPID &pid, int goalSpeed, int realSpeed, int motornum)
{
  ...
  if(u!=pid.pred_u)
  {
    pid.pred_u = u;
    if(motornum==0)
      robot.fptrMotorLeftGo(u);
    else
      robot.fptrMotorRightGo(u);
  }
}
...
// Тест подключения двигателей
{
  ...
  robot.goFastLeft(pwm);
  delay(TM);

  PSERIAL.println("go Fast right");
  robot.goFastRight(pwm);
  delay(TM);
