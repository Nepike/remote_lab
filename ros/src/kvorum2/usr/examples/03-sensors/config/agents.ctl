...
def CreateRobot(id, pos, tip, show_id):
    ...
    # Сенсоры
    #
    # Аргументы: caddr, cid,  cdir, cang, cR, cLevel, ctip, cvaltype = gdic.ST_SCALAR, cfproc = None, cval = 0
    #    cname -  Имя
    ...
    #
    A.AddSens(TSensor("IR_DETECTOR", 0, 0, 0, gdic.LEVEL_IR, gdic.ST_DETECTOR))

    #
    # Локатор
    #
    # Тип датчика - gdic.RST_VECTOR
    # Работает со слоем gdic.LEVEL_GROUND
    cdir = 0
    cang=180
    DIST = 20
    A.AddSens(TSensor("locator", cdir, cang, DIST, gdic.LEVEL_GROUND, gdic.ST_USONIC, gdic.RST_VECTOR))
