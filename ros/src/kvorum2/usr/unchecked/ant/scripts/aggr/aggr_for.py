...
MAXSTEPS = 10000  # количество шагов моделирования
...
def main(envfilename, mapfilename, agentfilename):
    ...
    while not rospy.is_shutdown():
        ...
        for a in tmu.Robots:
            ...
            if plcolor==a.agent.ahome[2]:  
                #a.agent.status = S_WALKING
                if a.agent.id == RAZMER: print a.agent.id, ": I am at home ", plcolor
            if a.agent.status <> S_FORAGING:  # если не фуражир - ищет еду
                if (plcolor == FoodColor1 or plcolor == FoodColor2):
                    a.EatProc()
                    a.agent.status = S_FORAGING # нашел еду - должен отнести ее в муравейник
                    if a.agent.id == RAZMER: print "status=", a.agent.status
                    Fgr[NoGrp] += S   # суммировать расстояние от дома до еды по группам
                    if NoGrp == plcolor: a.agent.pp_my += 1
                    else: a.agent.pp_notmy += 1
                    #if(JobREGIM==DEBUG): 
                    if a.agent.id == RAZMER: 
                        print "Датчик еды", a.agent.id, ": ", plcolor, " my - ", a.agent.pp_my, " notmy - ", a.agent.pp_notmy

            # Препятствие
            if(sGetFL(a)<5 or sGetFR(a)<5):
                if a.agent.status <> S_FORAGING:  # если не фуражир
                # вместо автомата - принудительный поворот на 180 градусов: не успевает отработать поворот при отходе от препятствия
                    ...
                    if(sGetFL(a)<5): tmu.SetAgentPos(a.agent.id, a.agent.pos[0], a.agent.pos[1], a.agent.pos[2]-90 )
                    else: tmu.SetAgentPos(a.agent.id, a.agent.pos[0], a.agent.pos[1], a.agent.pos[2]+90 )
                """
                else: 
                    #if(JobREGIM==DEBUG): 
            ...
            if(a.agent.status == S_FORAGING): # and not(sGetFL(a)<5 or sGetFR(a)<5)):
                if(a.FSM.Name != "ComeBack"):
                    #if(JobREGIM==DEBUG): 
                    print "load fsm_comeback"
                    a.FSM = LoadFSM(a, "ComeBack")          
            ...
            if a.agent.id == RAZMER: 
                GlobalTimer += 1
                print "***", a.agent.id, a.agent.pos[0], a.agent.pos[1], a.agent.pos[2]
                print  GlobalTimer, a.agent.id , "color-", plcolor, "status-", a.agent.status, "dist to home-", S
            r.sleep()
