...
def CreateRobot(id, pos, tip):
    ...
    A.Sensors.append(TSensor(gdic.ADDR_I2CDataServer, 1, 0, 0, 0, gdic.LEVEL_COLOR, gdic.ST_DETECTOR))

    # Суперлокатор. Он должен быть один
    #A.Sensors.append(TSensor(gdic.ADDR_SuperLocator,  0, 0, 180, 15, gdic.LEVEL_COLOR, gdic.ST_USONIC, gdic.RST_SUPER_VECTOR))
...
for n in range(0, A):
    ...
    x = X1 + 3*math.cos(fi)
    y = Y1 + 3*math.sin(fi)
    Agents.append(CreateRobot(k, [ x, y, grad+5 ], "turtle"))
    k +=1    
...
for n in range(0, B):
    ...
    x = X2 + 3*math.cos(fi)
    y = Y2 + 3*math.sin(fi)
    Agents.append(CreateRobot(k, [ x, y, grad+5 ], "elefant"))
    k +=1
