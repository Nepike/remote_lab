...
for n in range(0, A):
    ...
    x = X1 + 3*math.cos(fi)
    y = Y1 + 3*math.sin(fi)
    Agents.append(CreateRobot(k+RAZMER*0, [ x, y, grad ], "turtle"))
    x = X2 + 3*math.cos(fi)
    y = Y2 + 3*math.sin(fi)
    Agents.append(CreateRobot(k+RAZMER*1, [ x, y, grad ], "elefant"))
    x = X3 + 3*math.cos(fi)
    y = Y3 + 3*math.sin(fi)
    Agents.append(CreateRobot(k+RAZMER*2, [ x, y, grad ], "elefant"))
    x = X4 + 3*math.cos(fi)
    y = Y4 + 3*math.sin(fi)
    Agents.append(CreateRobot(k+RAZMER*3, [ x, y, grad ], "elefant"))
    k +=1   
